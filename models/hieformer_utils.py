...
def chunk_to_hidden(chunks,window_size = None,seq_length=1024):
    #chunks: (#chu,B,D)
    num_chunk,B = chunks.shape[0],chunks.shape[1]
    chunks = chunks.unsqueeze(-1)
    chunks = chunks.expand(-1,-1,-1,window_size) #(#chu,B,D,window)
    chunks = chunks.permute(1,2,3,0)

    chunks = chunks.contiguous().view(B,-1,num_chunk)
    chunks = F.fold(chunks,output_size = (seq_length,1),kernel_size=(window_size,1),stride = (window_size//2,1)) # (B,D,seq,1)
    hidden_states = chunks.squeeze().permute(2,0,1)
    return hidden_states #(sequence,B,D)

def hidden_to_chunk(hidden_states, key_padding_mask, window_size = None):
    #hidden_states: (Seq,B,D)
    #key_padding_mask: (B,Seq)
    key_padding_mask = key_padding_mask==False
   
    chunk_states = hidden_states.unfold(dimension = 0,size = window_size,step = window_size//2) # chunks,B,D,chunkLen
    chunk_states = torch.sum(chunk_states,axis = 3)# chunks,B,D

    chunk_padding_mask = key_padding_mask.unfold(dimension = 1,size = window_size,step = window_size//2)
    chunk_padding_mask = chunk_padding_mask.unsqueeze(2).permute(1,0,2,3)
    chunk_padding_mask = torch.sum(chunk_padding_mask,axis = 3)   # chunks,B,1
    mask = (chunk_padding_mask != 0)
    chunk_padding_mask = chunk_padding_mask.expand(-1,-1,chunk_states.shape[-1])# chunks,B,D
    mask_ = chunk_padding_mask != 0 
    chunk_states[mask_] =  chunk_states[mask_]/chunk_padding_mask[mask_]

    return chunk_states, mask #chunks,B,D, #chunks,B,1
