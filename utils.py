...
def my_collate(batch,pad_to_maxlen = True):
    ids = [ele["ids"] for ele in batch]
    mask = [ele["mask"] for ele in batch]
    token_type_ids = [ele["token_type_ids"] for ele in batch]

    bert_ids = pad_sequence(ids).permute(1, 0).long()
    bert_mask = pad_sequence(mask).permute(1, 0).long() #(B, #tokens)
    bert_token_type_ids = pad_sequence(token_type_ids).permute(1, 0).long()
    targets = [data["targets"] for data in batch] # length: 8
    targets = torch.stack( targets).long()
    if pad_to_maxlen:
        maxlen = 1024
        bert_ids = torch.nn.functional.pad(bert_ids, (0,maxlen-bert_ids.shape[1]), "constant", 0) 
        bert_mask = torch.nn.functional.pad(bert_mask, (0,maxlen-bert_mask.shape[1]), "constant", 0) 
        bert_token_type_ids = torch.nn.functional.pad(bert_token_type_ids, (0,maxlen-bert_token_type_ids.shape[1]), "constant", 0) 
    return bert_ids,bert_mask,bert_token_type_ids,targets
...
def train_loop_fun1(data_loader, model, optimizer, device, scheduler=None):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        token_type_ids = [data["token_type_ids"] for data in batch]
        targets = [data["targets"] for data in batch] # length: 8
        target_labels = torch.stack( targets).long().to(device)
        optimizer.zero_grad()
